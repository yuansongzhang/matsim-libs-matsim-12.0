...
public class ConfigUtils implements MatsimExtensionPoint {
    ...
    /**
     * A standard version, where we ignore the 1st argument (which should be the
     * config file), and take everything afterwards. Standard usage should be
     * something like
     *
     * <pre>
    ...
    public static CommandLine getCommandLine(String[] args) {
        ...
        try {
            return new CommandLine.Builder(typedArgs).allowPositionalArguments(false).allowAnyOption(true).build();
    ...
    /**
     * This variant is meant such that one can have a command line call to MATSim
     * that first provides a config file, and then overrides some of it. Should be
     * particularly useful for integration tests for main methods, since, if those
     * main methods are using this method here, it should be possible to test them
     * via
     *
     * <pre>
     ...
     * </pre>
     * <p>
     * i.e. the current necessity to break runnable scripts into pieces to change
     * things like output directory or lastIteration should be gone with this.
     */
    ...
    /**
     * This does (hopefully) overwrite config settings if they are defined in the
     * file. So you can do
     *
     * <pre>
    ...
    public static void loadConfig(final Config config, final String filename) throws UncheckedIOException {
        if (config.global() == null) {
            ...
            // if the config does already exist, this can be used to read a second config
            // file to override
            // settings from before, e.g. using a base config and then several case study
            // configs.
            ...
            // now also take it from the config file root when it is called from the command
            // line
    ...
    /**
     * Adds the <code>pathPrefix</code> in front of all supported file-paths in
     * core-config groups in the configuration, except when the paths are specified
     * as absolute paths. This is useful e.g. if the configuration contains
     * file-paths relative to the location of the config-file. The currently
     * supported file-paths are:
     * <ul>
    ...
    private static String getAbsolutePath(final String prefix, final String path) {
        ...
        /*
         * even if the file exists at the given location, its path seems not to be
         * specified as absolute path, thus we have to interpret it as a relative path
    ...
    @Deprecated // using this vs not using this can change results, presumably because
    // strategies may be used in a different sequence from the registry.
    ...
    /**
     * This is a refactoring device to remove former core config groups from the
     * core config. Instructions: If you want to remove e.g.
     * Config.vspExperimental(), replace that method with
