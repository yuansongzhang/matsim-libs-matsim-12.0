...
class ScenarioLoaderImpl {
	...
	/**
	 * Loads all mandatory Scenario elements and if activated in config's scenario
	 * module/group optional elements.
	 * 
	 * @return the Scenario
	 */
	Scenario loadScenario() {
		...
		// the above is not used and thus only causing confusion in the log output. kai,
		// sep'18
	...
	private void loadNetwork() {
		if ((this.config.network() != null) && (this.config.network().getInputFile() != null)) {
			...
			MatsimNetworkReader reader = new MatsimNetworkReader(inputCRS, config.global().getCoordinateSystem(),
	...
	private void loadActivityFacilities() {
		if ((this.config.facilities() != null) && (this.config.facilities().getInputFile() != null)) {
			...
			MatsimFacilitiesReader reader = new MatsimFacilitiesReader(inputCRS, internalCRS,
		...
		if ((this.config.facilities() != null)
			...
			URL facilitiesAttributesURL = ConfigGroup.getInputFileURL(this.config.getContext(),
					this.config.facilities().getInputFacilitiesAttributesFile());
			log.info("loading facility attributes from " + facilitiesAttributesURL);
			parseObjectAttributesToAttributable(facilitiesAttributesURL,
	...
	private void loadPopulation() {
		...
		if ((this.config.plans() != null) && (this.config.plans().getInputPersonAttributeFile() != null)) {
			...
			parseObjectAttributesToAttributable(personAttributesURL, scenario.getPopulation().getPersons().values(),
			...
			// // since ScenarioLoader is supposed to only read material, there are cases
			// where the output directory does not exist at
			// // this stage. One could maybe write to the "config.getContext()" directory.
			// However, sometimes this is a URL, and thus also
			// // non-writeable, and it is even less systematic than writing into the output
			// directory. kai, jun'19
			//
			// String outFilename = outputDirectory +
			// "/input_plans_with_person_attributes.xml.gz";
			// PopulationUtils.writePopulation( scenario.getPopulation(), outFilename );
			//
			// log.warn(
			// "a file with path=" + outFilename + " was just written in order to facilitate
			// the transition to having person attributes inside " +
			// "the persons. " );
			// }
	...
	private void loadHouseholds() {
		...
		if ((this.config.households() != null) && (fn != null)) {
			...
			parseObjectAttributesToAttributable(householdAttributesFileName,
	...
	private void loadTransit() throws UncheckedIOException {
		...
		if (this.config.transit().getTransitLinesAttributesFile() != null) {
			...
			URL transitLinesAttributesFileName = IOUtils.extendUrl(this.config.getContext(),
					this.config.transit().getTransitLinesAttributesFile());
			log.info("loading transit lines attributes from " + transitLinesAttributesFileName);
			parseObjectAttributesToAttributable(transitLinesAttributesFileName,
		...
		if (this.config.transit().getTransitStopsAttributesFile() != null) {
			...
			URL transitStopsAttributesURL = IOUtils.extendUrl(this.config.getContext(),
					this.config.transit().getTransitStopsAttributesFile());
			log.info("loading transit stop facilities attributes from " + transitStopsAttributesURL);
			parseObjectAttributesToAttributable(transitStopsAttributesURL,
	...
	private <T extends Identifiable<?> & Attributable> void parseObjectAttributesToAttributable(URL url,
			Iterable<T> attributables, String message) {
		...
		for (T facility : attributables) {
			Collection<String> keys = ObjectAttributesUtils.getAllAttributeNames(attributes,
		...
		// (some of the above could also become a static helper method in
		// ObjectAttributesUtils, but this here seems the only
		// place within matsim core where the personAttributes are automatically read so
		// maybe there is no need for this. kai, jun'19)
