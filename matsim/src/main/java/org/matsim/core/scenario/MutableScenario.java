 ...
 * learning */ 
 public final class MutableScenario implements Scenario, Lockable {
	...
	// NOTE: Thibaut is not so happy about just returning null, since someone may
	// code something like
	// Vehicles vehicles = scenario.getVehicles() ; // returning null
	// ... // lots of code
	// ... vehicles.getVehicles() ... // throwing a null pointer exception
	// That is, in such a situation the null pointer exception is thrown much later
	// than when the null pointer was obtained.
	// He rather wants to have this fail when it is called.
	// I (kn) am a bit sceptic if it makes sense to establish such a convention when
	// most other people use "if ... == null" use as the
	// official dialect to check if something is there (since this would cause an
	// exception with Thibaut's approach). kai, feb'15
	...
	public final void addScenarioElement(final String name, final Object o) {
		// Once the "removal" operation is locked, you cannot add under the same name.
		// kai, sep'14
		if (o == null)
	...
	private void testForLocked() {
		if (locked) {
			...
			/*
			 * The decision is roughly as follows: - It is ok to set network, population,
			 * etc. in the Scenario during initial demand generation. - It is NOT ok to do
			 * this once the services is running. - But we do not want to make a defensive
			 * copy of the whole thing at services startup. - We also want to be able to
			 * plug alternative Scenario implementations into the services. - But then the
			 * services only gets the "Scenario" not the "MutableScenario", so it does not
			 * have to worry about setNetwork and the like since it does not exist in the
			 * published interface. kai, sep'14
			 */
		}
	}
